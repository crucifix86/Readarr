...
namespace NzbDrone.Core.MediaFiles.BookImport.Specifications
{
    public class CloseBookMatchSpecification : IImportDecisionEngineSpecification<LocalEdition>
    {
        ...
        {
            if (_configService.SkipBookMatching)
            {
                _logger.Debug($"Skip-book-matching is enabled; accepting {item} without distance check");
                return Decision.Accept();
            }

            var threshold = _configService.BookMatchThreshold;
            double dist;
            string reasons;
            ...
            _logger.Debug($"Accepting release {item}: dist {dist} vs {threshold} {reasons}");
            return Decision.Accept();
        }
    }
}
